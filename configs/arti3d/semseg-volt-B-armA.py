"""Track 2 base config: point-level 3-class semantic segmentation on a Volt-B backbone.

Classes are {background, rotation-handle, translation-handle} — a handle's class is its parent
part's motion type. There are no queries and no superpoint pooling here, deliberately: interactable
handles are 16-21 points, a query decoder loses badly at that size, and superpoint pooling destroys
about a fifth of the handle training targets outright.

  * cross-entropy weights [0.1, 1, 1] against a 0.1%-foreground target, plus a Lovasz term.
  * a coarse-to-fine label curriculum: targets are dilated 0.10 m early in training and relaxed to
    the raw labels by the end, so the network first learns where handles are and then learns their
    extent.
  * voxel grid chosen so a handle does not share voxels with the door it sits on.

Judge this model on instance AP50 through the connected-component stage, not on mIoU: mIoU is
dominated by the background class and moves in the opposite direction from the metric that counts.
"""
...
# Coarse-to-fine GT dilation. MUST end at 0.0 so the final model trains on the same undilated
# labels the evaluator scores; otherwise it learns systematically fat masks, which at IoU 0.5 on
# ~20-point handles is exactly the bias that drops matches below threshold.
# Verified against the shipped expand_dict: distances span 0.0082-0.1000 m, so r=0.10 adds 735
# points on the sample scene, r=0.04 adds 135, r=0.00 adds none.
# FRACTIONS of training, not epochs: Pointcept's trainer.epoch tops out at eval_epoch
# (=20 here), so an absolute schedule keyed at 100/160 would never fire.
...
dataset_type = "ArtiC2FDataset"     # DefaultDataset + epoch-dependent GT dilation
